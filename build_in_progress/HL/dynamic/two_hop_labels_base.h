...
PPR_type PPR_595;//具体是vector<vector<pair<int, vector<int>>>>
...
void two_hop_clear_global_values() {//作用是清空全局变量，具体实现是用swap函数将全局变量的值清空
...
class graph_hash_of_mixed_weighted_two_hop_case_info_v1 {//用于存储2-hop label的信息
//此外，还有一个PPR_type PPR，用于存储PPR
public:
	...
	long long int compute_L_bit_size() {//计算L的大小，单位是bit
	...
	long long int compute_PPR_bit_size() {//计算PPR的大小，单位是bit
		long long int size = 0;
...
bool compare_two_hop_label_small_to_large(two_hop_label_v1& i, two_hop_label_v1& j)//排序函数
{
...
// this function is used to query the distance between source and terminal in the graph with 2-hop labels L
	global_query_times++;

	/*return std::numeric_limits<double>::max() is not connected*/
	//connected是指source和terminal之间有路径
	if (source == terminal) {
		return 0;
	}
	...
	//用于遍历source和terminal的2-hop label，找到公共的hub。内部是有序的。
...
//reduc4的作用是找到两个2-hop label的公共hub，和它们之间的距离
	global_query_times++;
