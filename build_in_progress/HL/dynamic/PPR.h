...
//用二分法插入key到input_vector中，返回key的位置。
	...
	//这里是处理key不在vector中的情况，即left > right，具体来说，left = right + 1；然后应该插入在right和left之间
...
std::vector<int> PPR_retrieve(PPR_type& PPR, int v1, int v2) {//检索PPR(v1, v2)

	/*retrieve PPR(v1, v2)*/

	int pos = graph_hash_of_mixed_weighted_binary_operations_search_position(PPR[v1], v2);
	if (pos == -1) {//没有找到，返回空的vector<int>
		std::vector<int> x;
		return x;
	}
	else {
...
void PPR_replace(PPR_type& PPR, int v1, int v2, std::vector<int>& loads) {
	//这里是将PPR(v1, v2)的取值更新替换为loads
	/*replace PPR(v1, v2) = loads*/

	int pos = graph_hash_of_mixed_weighted_binary_operations_search_position(PPR[v1], v2);
	if (pos == -1) {//如果PPR(v1, v2)不存在，那么创建一个新的vector<int>，然后插入loads
...
void PPR_erase(PPR_type& PPR, int v1, int v2, int v3) {
	//这里是将v3从PPR(v1, v2)中删除，应该是默认有这条边了才会删除，所以没有判断是否存在。
	int pos = graph_hash_of_mixed_weighted_binary_operations_search_position(PPR[v1], v2);
