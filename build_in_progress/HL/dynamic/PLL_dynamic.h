...
void PLL_dynamic(graph_hash_of_mixed_weighted& input_graph, int max_N_ID, int num_of_threads, graph_hash_of_mixed_weighted_two_hop_case_info_v1& case_info){
	...
	if (max_N_ID > max_N_ID_for_mtx_595) {//这个是用于判断是否有足够的锁，来保护所有的节点，实现并行
		cout << "max_N_ID > max_N_ID_for_mtx_595; max_N_ID_for_mtx_595 is too small!" << endl;
		exit(1);
	}
	mtx_595[max_N_ID_for_mtx_595 - 1].lock();
	if (this_parallel_PLL_PSL_is_running_595 == true){//这个是当前的并行PLL是否在运行，如果true，会报错
	...
	/*change graphs*/
	//这是将输入的图转换成一个理想图，理想图是一个vector<vector<pair<int, weightTYPE>>>，每个节点的邻接表是按照节点ID从小到大排好序的
	...
	P_dij_595.resize(num_of_threads);//这个是用于存储每个线程的P值
	T_dij_595.resize(num_of_threads);//这个是用于存储每个线程的T值
	//P和T是清北（bushi）是用于dij算法的。
	Q_handles_dij_595.resize(num_of_threads);//这个是用于存储每个线程的优先队列的句柄
	queue<int>().swap(Qid_595);//存储每个线程的ID
	...
	for (int v_k = 0; v_k < N; v_k++) {
		if (ideal_graph_595[v_k].size() > 0) {  // not from isolated vertices，有邻接点的节点才会有标签
			results.emplace_back(
				pool.enqueue([v_k, N] { // pass const type value j to thread; [] can be empty
					PLL_thread_function_dij_mixed(v_k, N);//这个是调用dij算法的主要函数，用于生成标签
	...
	for (int i = 0; i < max_N_ID; i++) {
		...
		//根据标签的距离从小到大排序 return i.vertex<j.vertex
	}
	//赋值给case_info
